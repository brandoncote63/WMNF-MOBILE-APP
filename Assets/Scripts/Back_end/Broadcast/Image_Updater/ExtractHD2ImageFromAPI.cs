...
public class ExtractHD2ImageFromAPI : MonoBehaviour
{
    ...
    private IEnumerator LoadRssXml()
    {
        using (WWW www = new WWW(apiUrl))
        {
            ...
            if (string.IsNullOrEmpty(www.error))
            {
                ...
                // Parse the XML to extract the image URL for "WMNF HD2 The Urban Caf�"
                broadcastChannelImageUrl = ExtractImageUrlFromXml(rssXml, "https://www.wmnf.org/channels/the-urban-cafe/");
    ...
    private string ExtractImageUrlFromXml(string xml, string title)
    {
        try
        {
            ...
            XmlNode itemNode = xmlDoc.SelectSingleNode($"//item[link='{title}']");
            if (itemNode != null)
            {
